...
public class PostRequestUseCase {
    ...
    public Single<OcRepresentation> execute(String deviceId, String uri, List<String> resourceTypes,
                                            List<String> interfacesList, OcRepresentation rep) {
        return iotivityRepository.getDeviceCoapsIpv6Host(deviceId)
                .flatMap(host -> iotivityRepository.constructResource(
                        host,
                        uri,
                        resourceTypes,
                        interfacesList
                )).flatMap(ocResource -> iotivityRepository.post(ocResource, true, rep));
    }
}
