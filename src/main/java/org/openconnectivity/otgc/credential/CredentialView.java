...
public class CredentialView implements FxmlView<CredentialViewModel>, Initializable {
    ...
    @FXML private ListView<OicSecCred> cmsListView;
    ...
    public void handleDeleteCredential() {
        viewModel.deleteCred(viewModel.deviceProperty.get().getDeviceId(), cmsListView.getSelectionModel().getSelectedItem().getCredID());
    }
    ...
    private void processRetrieveCredsResponse(ObservableValue<? extends Response<OicSecCreds>> observableValue, Response<OicSecCreds> oldValue, Response<OicSecCreds> newValue) {
        switch (newValue.status) {
            case LOADING:
                notificationCenter.publish(NotificationConstant.SET_PROGRESS_STATUS, true);
                break;
