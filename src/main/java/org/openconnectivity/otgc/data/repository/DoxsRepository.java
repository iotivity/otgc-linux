...
public class DoxsRepository {
    ...
    private final Logger LOG = Logger.getLogger(DoxsRepository.class);
    ...
    public Completable doOwnershipTransfer(String deviceId, OcfOxmType oxm) {
        return Completable.create(emitter -> {
            ...
            OCObtDeviceStatusHandler handler = (OCUuid ocUuid, int status) -> {
                if (status >= 0) {
                    LOG.debug("Successfully performed OTM on device " + OCUuidUtil.uuidToString(ocUuid));
                    emitter.onComplete();
                } else {
            ...
            if (ret >= 0) {
                LOG.debug("Successfully issued request to perform ownership transfer");
            } else {
    ...
    public Completable resetDevice(String deviceId) {
        return Completable.create(emitter -> {
            ...
            OCObtDeviceStatusHandler handler = (OCUuid ocUuid, int status) -> {
                if (status >= 0) {
                    LOG.debug("Successfully performed hard RESET to device " + OCUuidUtil.uuidToString(ocUuid));
                    emitter.onComplete();
                } else {
            ...
            if (ret >= 0) {
                LOG.debug("Successfully issued request to perform hard RESET");
            } else {
