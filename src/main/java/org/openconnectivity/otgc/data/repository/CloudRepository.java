...
public class CloudRepository {
    ...
    private final Logger LOG = Logger.getLogger(CloudRepository.class);
    ...
    @Inject
    public CloudRepository(SettingRepository settingRepository,
                           DeviceDao deviceDao) {
    ...
    private OCCloudHandler handler = (ctx, status) -> {
        LOG.debug("Cloud Manager Status:");
        ...
        if ((status & OCCloudStatusMask.OC_CLOUD_TOKEN_EXPIRY) == OCCloudStatusMask.OC_CLOUD_TOKEN_EXPIRY) {
            LOG.debug("\t\t-Token Expiry: ");
            if (ctx != null) {
                LOG.debug("%d\n", this.retrieveTokenExpiry().blockingGet());
            }
        }
    ...
    public Completable register() {
        return Completable.create(emitter -> {
            ...
            if (ret < 0) {
                ...
                LOG.debug("Issued Cloud Register request.");
            }
        })
    ...
    public Completable deregister() {
        return Completable.create(emitter -> {
            ...
            if (ret < 0) {
                ...
                LOG.debug("Issued Cloud Deregister request.");
            }
        })
        .timeout(getDiscoveryTimeout(), TimeUnit.SECONDS)
        .onErrorComplete();
    ...
    public Completable login() {
        return Completable.create(emitter -> {
            ...
            if (ret < 0) {
                ...
                LOG.debug("Issued Cloud Login request.");
            }
        })
        .timeout(getDiscoveryTimeout(), TimeUnit.SECONDS)
        .onErrorComplete();
    ...
    public Completable logout() {
        return Completable.create(emitter -> {
            ...
            if (ret < 0) {
                ...
                LOG.debug("Issued Cloud Logout request.");
            }
        })
        .timeout(getDiscoveryTimeout(), TimeUnit.SECONDS)
        .onErrorComplete();
    ...
    public Completable refreshToken() {
        return Completable.create(emitter -> {
            ...
            if (ret < 0) {
                ...
                LOG.debug("Issued refresh token request.");
            }
        })
        .timeout(getDiscoveryTimeout(), TimeUnit.SECONDS)
        .onErrorComplete();
