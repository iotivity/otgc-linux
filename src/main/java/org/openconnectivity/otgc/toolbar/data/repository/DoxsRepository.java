...
package org.openconnectivity.otgc.toolbar.data.repository;
        ...
        return Completable.create(emitter -> {
            try {
                ocSecureResource.doOwnershipTransfer((results, hasError) -> {
                    if (hasError == 0) {
                        LOG.debug("Ownership Transfer succeeded");
                        emitter.onComplete();
                    } else {
                        if (!results.isEmpty()) {
                            LOG.debug("Ownership Transfer failed: " + results.get(0).getResult());
                        }
                ...
                ocSecureResource.resetDevice(timeout,
                        (results, hasError) -> {
                            if (hasError == 0) {
                                ...
                                LOG.error("Error reseting device");
